...
const wifi_promiscuous_filter_t filt={ // This is the filter for the packets. This is where you can change what packets are sniffed.
...
void updatetime(){ // This updates the time the device has been online for
  for(int i=0;i<=63;i++){
    if(!(maclist[i][0] == "")){
      ...
      if(!(maclist[i][2] == "NO Probe request detected!")){
...
void showpeople(){ // This checks if the MAC is in the recognized list and then displays it on the OLED and/or prints it to serial.
  String forScreen = "";
  for(int i=0;i<=63;i++){
...
void loop() {
    ...
    showpeople();
     lcd.setCursor(0, 0);
  // print message
    //lcd.print();  //so far its counting the same person multiple times 
    //delay(100);
    curChannel++;
}
